...
struct DSSVisibleField: Sendable, Equatable {
    let page: Int
    let originX: CGFloat
    let originY: CGFloat
    let width: CGFloat
...
struct PDFCoordinateConverter {
    func pageRect(_ cropBoxLocalRect: CGRect, in cropBox: CGRect) -> CGRect {
        cropBoxLocalRect.offsetBy(dx: cropBox.minX, dy: cropBox.minY)
    }

    func cropBoxLocalRect(_ pageRect: CGRect, cropBox: CGRect) -> CGRect {
