...
- Browser signing (`Sources/AutogramWebBridge/`, `Sources/autogram-webbridge-agent/`, `Sources/AutogramWebExtensionHandler/`, `WebExtension/`): a Safari web extension owns `window.ditec` on state portals and signs through this app. Transport is native messaging only, with no HTTP port: launchd owns Mach service names, so `autogram-webbridge-agent` (a LaunchAgent installed by `scripts/install-webbridge-agent.sh`) holds the name as a rendezvous, the app registers an anonymous `NSXPCListener` endpoint with it, and the sandboxed appex asks for that endpoint and then talks to the app directly. The agent starts the app on demand. Every request raises a floating panel (`WebSigningPrompt`) because a background app cannot take focus since macOS Sonoma; nothing signs without confirmation there, with a card or with a phone over NFC. The portal decides the format and asks for Baseline B; the qualified timestamp switch defaults to off. Machine protocol sign requests carry optional eForm and XDC attributes so `XDCBuilder` and the UPVS, ORSR and FS resolvers in the bundled engine are reachable from Swift, and `SigningParameters.buildParameters` is shared with the engine's own HTTP entry point so the two cannot drift. Wire format between extension and app is covered by `WebSignRequestWireFormatTests`: both sides encoding with Swift hides real mismatches. `scripts/safari-spike.sh` checks everything that does not need Safari; `swift run webbridge-probe --sign <file>` exercises the whole app-side path. Design: `Autogram/docs/superpowers/specs/2026-09-11-safari-extension-design.md`
