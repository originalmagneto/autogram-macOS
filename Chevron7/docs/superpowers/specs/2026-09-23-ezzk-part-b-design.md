...
- **Signature:** XAdES, one reference to the `.xdcf` plus SignedProperties, `DataObjectFormat` MIME `application/vnd.gov.sk.xmldatacontainer+xml`, exclusive C14N, RSA-SHA256, `SigningTime` and one `SignatureTimeStamp` (level Baseline T). The timestamp comes from the Belgian qualified TSA (BOSA, policy `2.16.56.13.6.3.1.1000`), which is the first entry of the app's `TimestampAuthority.builtIn` (`TimestampAuthority.swift:22`) and the app's default. The signing certificate is the advocate's I.CA mandate certificate ("OPRÁVNENIE 1042", policy `1.3.158.36061701.1.1.1042`, QC policy `0.4.0.194112.1.2`, QCP-n-qscd).
...
8. **Signing orchestration.** Two sign requests with the same PIN: client container, then record. The qualified timestamp is forced on outside Demo (the switch is not offered there), and outside Demo signing does not start unless the selected authority is qualified (`TimestampAuthority.isQualified`; `builtIn` still lists "Sectigo (nekvalifikovaná)", `TimestampAuthority.swift:26`). After signing, both results are inspected and must report `qualifiedTimestampValid`; otherwise nothing is sent.
...
- The main window's ASiC-E output, which nests the same way (`SigningSessionStore.swift:541,1117,1347,1375`, open item of `WEB-SIGNING-FINDINGS` finding 6). B1's multi-document engine capability is the fix; rewiring that path is a separate task after B1.
